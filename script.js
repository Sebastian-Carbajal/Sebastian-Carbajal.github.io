...
for (const div of divisasDisponibles) {
...
outputResult.innerHTML = `<input type="number"/>`
...
// Creación del texto que muestra las conversiones realizadas 
const textoFinal3 = document.createElement("p")
textoFinal3.style.backgroundColor = "white"
caja.append(textoFinal3)
...
function calcularConversion () {
    ...
    const cantAConvertir = quantityInput.value
        ...
        divisas.forEach(div => {
                    // Se crea un array para guardar las conversiones 
                    let conversiones = []
                    
                    // Se verifica si las conversiones realizadas se encuentran en el localStorage 
                    const currencyLS = localStorage.getItem('Conversion')
                    if(currencyLS !== null) { 
                        conversiones = JSON.parse(currencyLS) 
                    }
                    
                    // Con este bucle se logra mostrar en la pantalla de la app la última o más reciente conversión realizada   
                    for (const conversion of conversiones) {
                        setTimeout( () => { 
                            textoFinal3.innerHTML = `
                            <p>La última conversión hecha fue: ${conversion.cantidadAConvertir} ${conversion.divisaIn} = ${conversion.resultadoDeConversion} ${conversion.divisaOut}</p>
                            `
                        },400)
                    }
                    ...
                    textoFinal2.innerHTML = `${segundoSelect}`

                    // Se agregan las opciones que selecciona el user al array de conversiones 
                    conversiones.push({
                        cantidadAConvertir: parseFloat(cantAConvertir),
                        divisaIn: primerSelect,
                        divisaOut: segundoSelect,
                        resultadoDeConversion: parseFloat(outputResult.value), 
                    })
                    
                    // Se crea una clave en el localStorage para que almacene las conversiones 
                    localStorage.setItem('Conversion', JSON.stringify(conversiones))
